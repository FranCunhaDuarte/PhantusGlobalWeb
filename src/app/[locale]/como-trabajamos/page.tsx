import type { Metadata } from 'next';
import { getTranslations, setRequestLocale } from 'next-intl/server';
import { ID_CONTENIDO } from '@/components/layout/SaltarAlContenido';
import ComoTrabajamos from '@/components/sections/ComoTrabajamos';
import LlamadaAContacto from '@/components/sections/LlamadaAContacto';
import { alternativasDeRuta } from '@/lib/rutas';
import { tarjetaSocial } from '@/lib/tarjeta-social';

export async function generateMetadata({
  params
}: PageProps<'/[locale]/como-trabajamos'>): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'comoTrabajamos' });
  const tMetadata = await getTranslations({ locale, namespace: 'metadata' });
  const tMarca = await getTranslations({ locale, namespace: 'marca' });

  return {
    title: t('metaTitle'),
    description: t('metaDescription'),
    alternates: alternativasDeRuta('/como-trabajamos', locale),
    ...tarjetaSocial(
      '/como-trabajamos',
      locale,
      tMarca('nombre'),
      tMetadata('ogAlt')
    )
  };
}

export default async function ComoTrabajamosPage({
  params
}: PageProps<'/[locale]/como-trabajamos'>) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({
    locale,
    namespace: 'comoTrabajamos.contacto'
  });

  return (
    <main id={ID_CONTENIDO} className="flex-1">
      <ComoTrabajamos />
      <LlamadaAContacto
        titulo={t('titulo')}
        texto={t('texto')}
        cta={t('cta')}
      />
    </main>
  );
}
